  ...
  container.innerHTML = exercises.map((ex, i) => `
    <div class="card" style="margin-bottom:var(--space-md)">
      <h3 style="color:var(--color-primary);margin-bottom:var(--space-sm);font-size:1rem">
        Aufgabe ${i + 1}${ex.title ? ' – ' + ex.title : ''}
      </h3>
      <p style="color:var(--color-text-secondary);line-height:1.6;margin-bottom:var(--space-sm);font-size:0.9rem">${ex.question}</p>
      ${ex.task ? `<div style="background:var(--surface-2,var(--surface));border-radius:var(--radius-sm);padding:var(--space-sm) var(--space-md);margin-bottom:var(--space-sm)">${ex.task}</div>` : ''}
